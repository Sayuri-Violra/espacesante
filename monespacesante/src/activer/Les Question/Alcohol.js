...
const Alcohol = () => {
  ...
  const isValidInput = () => {
    const regex = /^\d*$/;
    return regex.test(numDays) && parseInt(numDays) >= 0 && parseInt(numDays) <= 7;
  };
  ...
  const button = (
    ...
    >
      <Link className="Repair" to="/Glass">
        Suivant
      </Link>
    </button>
    </div>
  ...
  const message = (
    <p className="Condition">
      La valeur doit être comprise entre 0 et 7 et un entier.
    </p>
  );
  ...
  return (
    <div>
      <div>
        <a href="/Lecture" className="Retour">
          <img src={Arrow} className="Arrow" /> &nbsp;
          Etape précédente
        </a>
        <progress className="Bar" id="loading-bar" value="90" max="100" ></progress>
        <label htmlFor="numDaysInput" className="Drink">
          Nombre de jours où j’ai bus de l’alcool au cours de la semaine dernière:
        </label>
        <br />
        <input
          type="text"
